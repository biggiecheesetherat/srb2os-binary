	...
	do
	{
		if (myfgets(s, MAXLINELEN, f))
		{
			...
			if (fastcmp(word, "LEVELNAME"))
			{
				...
				strlcpy(header->selectheading, word2, sizeof(header->selectheading)); // not deh_strlcpy so only complains once
				continue;
			}
			...
			{
				deh_strlcpy(header->forcecharacter, word2, sizeof header->forcecharacter,
			...
			{
				deh_strlcpy(header->interscreen, word2, sizeof header->interscreen,
