...
rhi::Rect srb2::hwr2::trimmed_patch_dimensions(const patch_t* patch)
{
	...
	{
		const column_t* column = &patch->columns[x];

		// If we haven't reached a column that has posts,
		if (!minx_found && column->num_posts == 0)
		{
			// the minx is at least one higher than the current column.
			minx = x + 1;
			continue;
		}
		minx_found = true;
		...
		miny = std::min(static_cast<int32_t>(column->posts[0].topdelta), miny);
		const post_t* last_post = &column->posts[column->num_posts - 1];
		maxy = std::max<int32_t>(last_post->topdelta + last_post->length, maxy);
	}
...
void srb2::hwr2::convert_patch_to_trimmed_rg8_pixels(const patch_t* patch, std::vector<uint8_t>& out)
{
	...
	{
		const column_t* column = &patch->columns[x + trimmed_rect.x];
		unsigned int num_posts = column->num_posts;

		for (unsigned int pi = 0; pi < num_posts; pi++)
		{
			const post_t* post = &column->posts[pi];

			const uint8_t* source = &column->pixels[post->data_offset];
			int32_t count = post->length;

			for (int32_t j = 0; j < count; j++)
			{
				int32_t output_y = post->topdelta + j - trimmed_rect.y;
				if (output_y < 0)
				{
					continue;
				}
				...
				out[pixel_index + 0] = source[j]; // index in luminance/red channel
