...
static int skin_get(lua_State *L)
{
	...
	case skin_skinsprites:
		LUA_PushUserdata(L, &skin->sprites[SKINSPRITES_BASE], META_SKINSPRITES);
		break;
	case skin_supersprites:
		LUA_PushUserdata(L, &skin->sprites[SKINSPRITES_SUPER], META_SKINSPRITES);
		break;
	case skin_natkcolor:
...
static int lib_getSkinSprite(lua_State *L)
{
	...
	if (sksprites->spritenum[i] == SPR_NULL)
		return 0;

	LUA_PushUserdata(L, &sprites[sksprites->spritenum[i]], META_SKINSPRITESLIST);
	return 1;
}
