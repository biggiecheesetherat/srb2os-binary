...
static void ArchiveSideEdge(save_t *save_p, const side_overlay_t *edge, UINT16 diff)
{
	P_WriteUINT16(save_p, diff);

	if (diff & LD_EDGETEX)
		P_WriteINT32(save_p, edge->texture);
	if (diff & LD_EDGEOFFX)
		P_WriteFixed(save_p, edge->offsetx);
	if (diff & LD_EDGEOFFY)
		P_WriteFixed(save_p, edge->offsety);
	if (diff & LD_EDGESCALEX)
		P_WriteFixed(save_p, edge->scalex);
	if (diff & LD_EDGESCALEY)
		P_WriteFixed(save_p, edge->scaley);
	if (diff & LD_EDGEREPEAT)
		P_WriteINT16(save_p, edge->repeatcnt);
	if (diff & LD_EDGEALPHA)
		P_WriteFixed(save_p, edge->alpha);
	if (diff & LD_EDGEBLEND)
		P_WriteUINT8(save_p, edge->blendmode);
	if (diff & LD_EDGEFLAGS)
		P_WriteUINT8(save_p, edge->flags);
}

static void ArchiveSide(save_t *save_p, const side_t *si, UINT32 diff, UINT16 edgediff[4])
{
	...
	if (diff & LD_SDOVERLAY1)
		ArchiveSideEdge(save_p, &si->overlays[0], edgediff[0]);
	if (diff & LD_SDOVERLAY2)
		ArchiveSideEdge(save_p, &si->overlays[1], edgediff[1]);
	if (diff & LD_SDOVERLAY3)
		ArchiveSideEdge(save_p, &si->overlays[2], edgediff[2]);
	if (diff & LD_SDOVERLAY4)
		ArchiveSideEdge(save_p, &si->overlays[3], edgediff[3]);
	if (diff & LD_SDLIGHT)
...
static void ArchiveLines(save_t *save_p)
{
	...
	for (i = 0; i < numlines; i++, spawnli++, li++)
	{
		...
		if (diff)
		{
			...
			if (diff & LD_SIDE1)
				ArchiveSide(save_p, &sides[li->sidenum[0]], side1diff, edgediff1);
			if (diff & LD_SIDE2)
				ArchiveSide(save_p, &sides[li->sidenum[1]], side2diff, edgediff2);
...
static void UnArchiveSideEdge(save_t *save_p, side_overlay_t *edge)
{
	UINT16 diff = P_ReadUINT16(save_p);

	if (diff & LD_EDGETEX)
		edge->texture = P_ReadINT32(save_p);
	if (diff & LD_EDGEOFFX)
		edge->offsetx = P_ReadFixed(save_p);
	if (diff & LD_EDGEOFFY)
		edge->offsety = P_ReadFixed(save_p);
	if (diff & LD_EDGESCALEX)
		edge->scalex = P_ReadFixed(save_p);
	if (diff & LD_EDGESCALEY)
		edge->scaley = P_ReadFixed(save_p);
	if (diff & LD_EDGEREPEAT)
		edge->repeatcnt = P_ReadINT16(save_p);
	if (diff & LD_EDGEALPHA)
		edge->alpha = P_ReadFixed(save_p);
	if (diff & LD_EDGEBLEND)
		edge->blendmode = P_ReadUINT8(save_p);
	if (diff & LD_EDGEFLAGS)
		edge->flags = P_ReadUINT8(save_p);
}
...
static void UnArchiveSide(save_t *save_p, side_t *si)
{
	...
	if (diff & LD_SDOVERLAY1)
		UnArchiveSideEdge(save_p, &si->overlays[0]);
	if (diff & LD_SDOVERLAY2)
		UnArchiveSideEdge(save_p, &si->overlays[1]);
	if (diff & LD_SDOVERLAY3)
		UnArchiveSideEdge(save_p, &si->overlays[2]);
	if (diff & LD_SDOVERLAY4)
		UnArchiveSideEdge(save_p, &si->overlays[3]);
	if (diff & LD_SDLIGHT)
...
//          Misc
// =======================================================================
static void P_ArchiveMisc(save_t *save_p, INT16 mapnum)
{
	...
	P_WriteINT16(save_p, emeralds+357);
