...
void R_CreateInterpolator_Polyobj(thinker_t *thinker, polyobj_t *polyobj)
{
	...
	interp->polyobj.oldvertices = static_cast<fixed_t*>(Z_CallocAlign(sizeof(fixed_t) * 2 * polyobj->numVertices, PU_LEVEL, NULL, 32));
	interp->polyobj.bakvertices = static_cast<fixed_t*>(Z_CallocAlign(sizeof(fixed_t) * 2 * polyobj->numVertices, PU_LEVEL, NULL, 32));
